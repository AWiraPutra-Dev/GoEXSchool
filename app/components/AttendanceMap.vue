...
let zoneMarker: L.Marker | null = null
let userMarker: L.Marker | null = null
let radiusCircle: L.Circle | null = null

// ===== Marker profesional: pin SVG dengan bangunan sekolah =====
function buildZoneIcon(logo?: string | null): L.DivIcon {
  const inner = logo
...
function locate() {
  if (!navigator.geolocation) {
    locError.value = 'Browser ini tidak mendukung GPS. Aktifkan izin lokasi browser.'
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    (err) => {
      locating.value = false
      locError.value = `Gagal membaca lokasi: ${err.message}. Pastikan GPS aktif dan izin lokasi diberikan.`
      emitLocation()
    },
