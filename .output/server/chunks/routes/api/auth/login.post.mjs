import { c as defineEventHandler, r as readBody, e as createError, p as prisma, l as generateToken, m as toInstitutionSummary } from '../../../_/nitro.mjs';
...
const login_post = defineEventHandler(async (event) => {
  ...
  if (role === "student") {
    user = await prisma.user.findFirst({
      ...
      include: { student: true, institution: true, extracurricularOperator: { select: { id: true, name: true } } }
    });
  } else {
    user = await prisma.user.findFirst({
      where: { username: identifier, role },
      include: { institution: true, extracurricularOperator: { select: { id: true, name: true } } }
    });
  }
  if (!user) {
